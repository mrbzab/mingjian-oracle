...
export function BaziEnhancement({ result, year, onYearChange, refreshToken = 0 }: { result: BaZiResult; year:number; refreshToken?:number; onYearChange:(year:number)=>void }) {
  ...
  return <section className="panel analysis-panel space-y-6" aria-busy={busy}>
    ...
    {analysis && <>
      <Tabs defaultValue="summary" className="analysis-tabs"><TabsList className="analysis-tab-list"><TabsTrigger value="summary">分析概览</TabsTrigger><TabsTrigger value="evidence">详细依据</TabsTrigger><TabsTrigger value="prompt">AI 提示词</TabsTrigger></TabsList><TabsContent value="summary" className="space-y-5">
      ...
      </TabsContent><TabsContent value="evidence" className="space-y-6">
      ...
      </TabsContent><TabsContent value="prompt"><div className="space-y-3"><Label htmlFor="analysis-prompt">AI 解读提示词（可编辑）</Label><p className="text-sm leading-6 text-muted-foreground">这是解读材料，尚未调用 AI。含当前出生资料与反馈；复制前可删除不想分享的信息。修改上方问题后，点击重新生成。</p><Textarea id="analysis-prompt" value={prompt} onChange={(e) => setPrompt(e.target.value)} className="min-h-80 text-sm leading-6" /><Button variant="outline" onClick={copy} disabled={!prompt.trim() || stale}>复制提示词</Button></div></TabsContent></Tabs>
    </>}
