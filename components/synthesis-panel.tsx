...
export function SynthesisPanel({result,year,ziwei,qimen,refreshToken=0}:{result:BaZiResult;year:number;ziwei:ZiweiResult|null;qimen:QimenResult|null;refreshToken?:number}) {
 ...
 const key=JSON.stringify({input:result.input,year,ziwei,qimen,question}),stale=!!prompt&&snapshot!==key;
 const generation=useRef(0);
 useEffect(()=>{generation.current++;return()=>{generation.current++;};},[key]);
 async function generate(){const request=++generation.current;setBusy(true);setError('');try{const {buildSynthesis}=await import('@/lib/synthesis');const next=await buildSynthesis(result,year,ziwei,qimen,question);if(request!==generation.current)return;setPrompt(next.prompt);setSnapshot(key);setNotice('完整综合资料已生成。');}catch(e){if(request===generation.current)setError(e instanceof Error?e.message:'生成失败。');}finally{setBusy(false);}}
 ...
 return <section className="panel space-y-6"><div><h3 className="section-title">综合解读摘要</h3><p className="leading-7 text-muted-foreground">根据当前有效命盘整理，摘要随资料同步；各体系的来源和时间范围分别列出。</p></div>
 ...
 <details className="rounded-xl bg-muted p-4"><summary className="cursor-pointer font-medium">摘要的范围与限制</summary><ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-7">{summary.limits.map(l=><li key={l}>{l}</li>)}</ul></details>
