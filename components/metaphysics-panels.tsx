...
export function ZiweiPanel({ result }: { result: BaZiResult }) {
  ...
  const key = JSON.stringify({ algorithm, question });
  const stale = !!data && key !== snapshot;
  async function generate() {
    setBusy(true); setError('');
    try { const engine = await import('@/lib/metaphysics'); const next = await engine.calculateZiwei(result, algorithm); setData(next); setPrompt(engine.expansionPrompt(next, question)); setSnapshot(key); }
  ...
  return <section className="panel space-y-6" aria-busy={busy}><div><h3 className="section-title">紫微斗数</h3><p className="text-sm leading-7 text-muted-foreground">沿用已排命盘的出生资料：{result.date} · {result.input.unknownTime ? '时刻未知' : result.input.time} · {result.input.city}。修改出生资料后，先点击“排出命盘”。</p></div>
    <div className="analysis-inputs"><div className="space-y-2"><Label htmlFor="ziwei-question">想了解的问题（选填）</Label><Textarea id="ziwei-question" maxLength={2000} value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="例如：命宫与三方四正怎样联系？" /></div><div className="space-y-2"><Label htmlFor="ziwei-school">排盘口径</Label><NativeSelect id="ziwei-school" value={algorithm} onChange={(e) => setAlgorithm(e.target.value as typeof algorithm)}><NativeSelectOption value="default">通行口径</NativeSelectOption><NativeSelectOption value="zhongzhou">中州派</NativeSelectOption></NativeSelect></div></div>
    ...
    </>}<p className="text-sm text-muted-foreground">来源：<a href="https://github.com/Brhiza/mingyu" className="source-link">命语</a> · <a href="https://github.com/SylarLong/iztro" className="source-link">iztro</a>。本页展示本命盘，未计算流年四化。</p></section>;
}

export function QimenPanel() {
  ...
  async function generate() {
    setBusy(true); setError('');
    try { const engine = await import('@/lib/metaphysics'); const next = await engine.calculateQimen(time); setData(next); setPrompt(engine.expansionPrompt(next, question)); setSnapshot(key); }
    catch (e) { setError(e instanceof Error ? e.message : '奇门起局未完成，请重试。'); }
    finally { setBusy(false); }
  }
  return <section className="panel space-y-6" aria-busy={busy}><div><h3 className="section-title">奇门遁甲</h3><p className="text-sm leading-7 text-muted-foreground">为所问之事选择起局时刻。时家转盘 · 拆补法 · 北京时间 UTC+8，独立于左侧出生资料。</p></div><div className="space-y-2"><Label htmlFor="qimen-time">起局时间（北京时间）</Label><div className="flex flex-wrap gap-3"><Input id="qimen-time" type="datetime-local" min="1901-01-01T00:00" max="2099-12-31T23:59" value={time} onChange={(e) => setTime(e.target.value)} className="max-w-xs" /><Button variant="outline" onClick={() => setTime(beijingNow().slice(0,16))}>使用当前北京时间</Button></div></div><div className="space-y-2"><Label htmlFor="qimen-question">所问之事（选填）</Label><Textarea id="qimen-question" maxLength={2000} value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="说明问题与背景，解读时将核对对应宫位" /></div><Button onClick={generate} disabled={busy || !time}>{busy ? '正在起局…' : data ? '重新生成奇门盘' : '生成奇门盘'}</Button>
    {error && <p role="alert" className="text-sm text-destructive">{error}</p>}{stale && <p role="status" className="text-sm text-primary">输入已修改，下方仍为上次结果，请重新生成。</p>}
    {data && <><div className="rounded-xl bg-muted p-5"><h4 className="font-serif text-2xl">{data.chart.isYangDun ? '阳遁' : '阴遁'} {data.chart.juShu} 局</h4><p className="mt-3 leading-7">值符 · {data.chart.zhiFu}　值使 · {data.chart.zhiShi}</p><p className="text-sm leading-7">{data.time.replace('T',' ')} UTC+8 · {Object.values(data.chart.ganzhi).join('　')}</p><p className="text-sm leading-7 text-muted-foreground">节气 {data.chart.timeInfo.solarTerm} · 定局节气 {data.chart.timeInfo.juTerm} · {data.chart.timeInfo.epoch} · 空亡 {data.chart.voidBranches.join('、') || '未提供'} · 驿马 {data.chart.horseStar?.branch ?? '未提供'}</p></div>
      <p className="text-sm text-muted-foreground">九宫按上南下北、左东右西排列；中宫寄宫情况以盘内随行星干为准。</p><div className="qimen-grid">{[4,9,2,3,5,7,8,1,6].map((number) => { const p = data.chart.jiuGongGe.find((item) => item.gong === number); return p ? <article key={number} className="qimen-palace"><h4 className="font-medium">{p.name} · {p.gong}</h4><p className="text-sm text-muted-foreground">{p.direction} · {p.element}</p><p className="my-3 font-medium text-primary">{p.renPan.door || '无门'} · {p.shenPan.god || '无神'}</p><p className="text-sm leading-6">九星 {p.tianPan.star || '—'}</p><p className="text-sm leading-6">天盘 {p.tianPan.stem || '—'} · 地盘 {p.diPan.stem || '—'}</p>{p.tianPan.companionStar && <p className="mt-2 text-sm leading-6">随行 {p.tianPan.companionStar} {p.tianPan.companionStem}</p>}</article> : null; })}</div>
      <details className="rounded-xl border p-4"><summary className="cursor-pointer font-medium">格局与天地盘干关系</summary><div className="mt-4 space-y-3 text-sm leading-7">{data.chart.patternDetails.map((p) => <p key={p.tag}>{p.tag}：{p.summary}</p>)}{data.chart.stemRelations.map((p, i) => <p key={i}>{p.gong}宫 · {p.heavenStem} / {p.earthStem} · {p.relation} {p.pattern}</p>)}</div></details><PromptBox id="qimen-prompt" prompt={prompt} setPrompt={setPrompt} stale={stale} />
