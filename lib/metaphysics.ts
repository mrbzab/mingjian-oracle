...
export async function calculateZiwei(result: BaZiResult, algorithm: 'default' | 'zhongzhou' = 'default', targetDate?: string) {
  ...
  let horoscope = null;
  if (targetDate) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) throw new Error('请选择完整的运限日期。');
    const date = Temporal.PlainDate.from(targetDate);
    if (date.year > 2199 || Temporal.PlainDate.compare(date, time.toPlainDate()) < 0) throw new Error('运限日期需在出生日期之后、2199 年以内。');
    const h = astrolabe.horoscope(targetDate, 6).toJSON();
    horoscope = { date: targetDate, nominalAge: h.age.nominalAge, decadal: h.decadal, yearly: h.yearly };
  }
...
export function ziweiPalaceEvidence(data: ZiweiResult, index: number) {
  const palace = data.chart.palaces.find((p) => p.index === index);
  if (!palace) throw new Error('宫位不存在。');
  const surrounded = [0,4,8,6].map((offset) => data.chart.palaces.find((p) => p.index === (index + offset) % 12)!);
  const natal = data.chart.palaces.flatMap((p) => [...p.majorStars, ...p.minorStars].filter((s) => s.mutagen).map((s) => ({ palace: p.name, star: s.name, transformation: s.mutagen })));
  const transformations = (names: string[] = []) => names.map((name, i) => ({ star: name, transformation: ['禄','权','科','忌'][i], palace: data.chart.palaces.find((p) => [...p.majorStars,...p.minorStars].some((s) => s.name === name))?.name ?? '未定位' }));
  return { palace, surrounded, natal, decadal: transformations(data.horoscope?.decadal.mutagen), yearly: transformations(data.horoscope?.yearly.mutagen) };
}
...
export function expansionPrompt(data: ZiweiResult | QimenResult, question: string) {
  return [
    ...
    '只分析提供的体系：紫微仅在 horoscope 有值时解释其中所选日期的大限与流年四化，并区分本命四化；奇门起局时刻不是出生时刻。未提供的八字、六爻、其他体系不得伪称已综合。使用短段落，不堆原始 JSON，不给虚构概率。',
