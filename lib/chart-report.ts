...
export function buildChartReport(result:BaZiResult,year:number,ziwei:ZiweiResult|null,qimen:QimenResult|null,options:ReportOptions):ChartReport {
 ...
 if(options.ziwei){if(!ziwei)sections.push({title:'紫微斗数',lines:['没有匹配当前出生资料、日期和流派的紫微命盘，请先生成。']});else{
  ...
  sections.push({title:'紫微斗数',lines:[ziwei.rules,`运限日期 ${ziwei.horoscope?.date??'未计算'}（农历年界）`,`命宫 ${soul.heavenlyStem}${soul.earthlyBranch} · 主星 ${soul.majorStars.map(s=>s.name).join('、')||'无主星'}`,`三方四正 ${e.surrounded.map(p=>p.name+' '+p.earthlyBranch).join('、')}`,...e.natal.map(r=>`本命 ${r.star}化${r.transformation} → ${r.palace}`),...(ziwei.horoscope?[`大限 ${ziwei.horoscope.decadal.heavenlyStem}${ziwei.horoscope.decadal.earthlyBranch} · 流年 ${ziwei.horoscope.yearly.heavenlyStem}${ziwei.horoscope.yearly.earthlyBranch}`]:[]),...e.decadal.map(r=>`大限 ${r.star}化${r.transformation} → ${r.palace}`),...e.yearly.map(r=>`流年 ${r.star}化${r.transformation} → ${r.palace}`)]});
