      ...
      <div className="workspace">
        <aside className="birth-panel"><fieldset disabled={refreshing} className="min-w-0">
          ...
          <form onSubmit={submit} className="space-y-4" onInvalidCapture={(event) => {
            ...
            <div className="space-y-2"><Label htmlFor="time">出生时刻（24 小时制）</Label><Input id="time" type="time" step="1" required={!input.unknownTime} disabled={input.unknownTime} value={input.time} onInput={(e) => update('time', e.currentTarget.value)} onChange={(e) => update('time', e.target.value)} className="h-11" /></div>
